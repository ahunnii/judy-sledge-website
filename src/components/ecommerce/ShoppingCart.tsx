...
const ShoppingCart = () => {
  ...
  const handleClick = async (event: MouseEvent<HTMLButtonElement>) => {
    ...
    if (cartCount && cartCount > 0) {
      setStatus("loading");
      try {
        const result = (await redirectToCheckout()) as CheckoutResult;
        if (result?.error) {
    ...
    <>
      ...
      >
        ...
        {Object.values(cartDetails ?? {}).length > 0 && (
          <sup className="flex h-5 w-5 items-center   justify-center rounded-full bg-purple-500 text-sm font-semibold text-white">
            {cartCount}
          </sup>
        )}
      </button>
